    ...
    await db.collection("produtos").get().then((snapshot)=>{
        ...
        Object.keys(produtos).forEach((produto, i)=>{
            tProdB.innerHTML += `<tr id="item-${produto}">
                                    ...
                                    <td class="price">${produtos[produto].preçoBase.toFixed(2)}</td>
                                    <td class="price"></td>
                                </tr>`
        })
    })
...
function calcular(preço){
    ...
    $(`#peso`).innerHTML = `Kg: ${pesoTot.toFixed(3)}`
    if(preço == "preçoBase"){
        let valueCalc = 0
        Object.keys(compras).forEach((id)=>{
            let value = produtos[id]["preço300"] * compras[id]
            valueCalc += value
        })
        if(valueCalc >= 300){
    ...
    if(preço == "preço300"){
        let valueCalc = 0
        Object.keys(compras).forEach((id)=>{
            let value = produtos[id]["preço600"] * compras[id]
            valueCalc += value
        })
        if(valueCalc >= 600){
            document.querySelectorAll(".price").forEach(element=>{
                element.style.backgroundColor = "rgb(46, 255, 46)"
            })
    ...
    if(preço == "preço600"){
        let valueCalc = 0
        Object.keys(compras).forEach((id)=>{
            let value = produtos[id]["preço1500"] * compras[id]
            valueCalc += value
        })
        if(valueCalc >= 1500){
            document.querySelectorAll(".price").forEach(element=>{
...
function createCadastro(){
    ...
    let cpf = $("#cpf").value
    formulario = `*Numero*:%20${numero}%0A*Bairro*:%20${bairro}%0A*Cep*:%20${cep}%0A*Cidade*:%20${cidade}%0A*Estado*:%20${estado}%0A*Tel*:%20${tel}%0A*Email*:%20${email}%0A*Endereço*:%20${address}%0A*CPF/CNPJ*:%20${cpf}%0A`
    enviar()
}
